 ...
 */
function addMetabox_slider () {
	add_meta_box( 
		'slider_caption', // id метабокса (прибумываю сам)
		'Слайдер', // создать слайдер
		'slider_caption_html',// вызываемая функция в которой отрисовываю код метабокса 
...
function slider_caption_html($post) { // отрисовывает html метабокса

// получение сохраннений слайдера и загрузка имеющихся в бд слайдов
$data = get_post_meta( $post->ID, '_slider__data', true );

if(empty($data)) { // если данных нет то записываю пустой массив
    $data = '[]';
}
...
?>
	...
	<div id="test" class="admin__slide">
        ...
        <component-settings title = 'Настройки слайдер'> </component-settings>
          <!-- <slider-button text="Заказать"></slider-button> -->
	</div>
	   
      
	<script>
        // модель с данными
	    <? require_once(plugins_url('/assets/js/component__model.js',__FILE__));  ?>
	   // UI вспомогательные элементы
	    <? require_once(plugins_url('/assets/js/UI_components.js',__FILE__));  ?>
        ...
        app.component('component-settings', setting_slider);
        app.component('slider-button', button_component);
        app.mount("#test");
	</script>
...
add_action('admin_menu','addMetabox_slider'); // добавляю метабокс
